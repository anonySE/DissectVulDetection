...
if __name__ == '__main__':
    ...
    param_grid = {
        'n_estimators': np.linspace(50, 150, 3).astype(int),
        'learning_rate': np.arange(0.9, 1.1, 0.1)
    }
    estimator = AdaBoostClassifier(DecisionTreeClassifier(max_features = 0.7, max_depth = 14,
                                    min_samples_split = 3, max_leaf_nodes = 40),
                                    algorithm = 'SAMME.R')
    ...
    FPRate = fp / (fp + tn)
    FNRate = fn / (fn + tp)
    print('\nAdaBoost classifier on training set:')
