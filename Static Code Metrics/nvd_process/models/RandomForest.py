...
if __name__ == '__main__':
    ...
    param_grid = {
        'n_estimators': np.linspace(185, 195, 3).astype(int),
        'max_depth': [None] + list(np.linspace(10, 12, 2).astype(int)),
        'max_features': ['auto', 'sqrt', 'log2', None],
        'max_leaf_nodes': [None] + list(np.linspace(35, 40, 2).astype(int)),
        'min_samples_leaf': [115, 120],
        'min_samples_split': [3, 4],
        'bootstrap': [True, False]
    }
    estimator = RandomForestClassifier(max_features = 'sqrt', class_weight = 'balanced')
