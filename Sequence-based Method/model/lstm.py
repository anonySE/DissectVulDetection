...
from tensorflow.keras.metrics import TruePositives, FalsePositives, FalseNegatives, Precision, Recall
...
def build_model(maxlen, vector_dim, layers, dropout):
    ...
    for i in range(1, layers):
        model.add(LSTM(units=512, activation='tanh', recurrent_activation='hard_sigmoid', return_sequences=True, use_bias=True))
        model.add(Dropout(dropout))

    model.add(LSTM(units=512, activation='tanh', recurrent_activation='hard_sigmoid', use_bias=True))
    model.add(Dropout(dropout))
...
if __name__ == "__main__":
    ...
    layers = 2
    dropout = 0.1
    traindataSetPath = "./dl_input/sard/train_3/"
    testdataSetPath = "./dl_input/sard/test_3/"
    print(traindataSetPath)
    realtestdataSetPath = "data/"
    weightPath = './model_gen/LSTM'
    resultPath = "./result/LSTM"
    main(traindataSetPath, testdataSetPath, realtestdataSetPath, weightPath, resultPath, batchSize, maxLen, vectorDim,
         layers, dropout)
    # testrealdata(realtestdataSetPath, weightPath, batchSize, maxLen, ivectorDim, layers, dropout)
