...
def build_model(maxlen, vector_dim, layers, dropout):
    ...
    for i in range(1, layers):
        model.add(Bidirectional(
            LSTM(units=512, activation='tanh', recurrent_activation='hard_sigmoid', return_sequences=True, use_bias=True)))
        model.add(Dropout(dropout))

    model.add(Bidirectional(LSTM(units=512, activation='tanh', recurrent_activation='hard_sigmoid', use_bias=True)))
    model.add(Dropout(dropout))
    ...
    model.compile(loss='binary_crossentropy', optimizer='adamax',
                  metrics=[
                      ...
                      tensorflow.metrics.FalsePositives(),
                      tensorflow.metrics.FalseNegatives(),
    ...
    model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
                filepath=weightpath,
                    save_weights_only=True,
                        ...
                        verbose=1)
    model.fit_generator(train_generator, steps_per_epoch=steps_epoch, epochs=10, callbacks=[model_checkpoint_callback])
    t2 = time.time()
    train_time = t2 - t1
...
if __name__ == "__main__":
    ...
    layers = 2
    dropout = 0.2
    traindataSetPath = "./dl_input/train_nvd_2/"
    testdataSetPath = "./dl_input/test_nvd_2/"
    print(traindataSetPath)
    realtestdataSetPath = "data/"
    weightPath = './model_gen/BLSTM_2'
    resultPath = "./result/BLSTM_2"
