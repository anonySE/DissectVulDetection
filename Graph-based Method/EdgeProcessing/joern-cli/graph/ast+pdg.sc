...
type r = (Option[String], List[aEdgeEntry], List[aVertexEntry], List[pEdgeEntry], List[pVertexEntry])
...
def result(methodRegex: String = ""): List[r] = {
  if (methodRegex.isEmpty) {
    ...
    List((None, aedgeEntries, avertexEntries, pedgeEntries, pvertexEntries))
  } else {
    cpg.method(methodRegex).l.map { method =>
      ...
      (Some(methodFile), aedgeEntries, avertexEntries, pedgeEntries, pvertexEntries)
    }
  }
}
...
@main def main()= {
  ...
  val dirPath = "raw_result//good_ast_pdg"
