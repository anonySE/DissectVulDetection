...
type r = (Option[String], List[cEdgeEntry], List[cVertexEntry], List[pEdgeEntry], List[pVertexEntry])
...
def result(methodRegex: String = ""): List[r] = {
  if (methodRegex.isEmpty) {
    ...
    List((None, cedgeEntries, cvertexEntries, pedgeEntries, pvertexEntries))
  } else {
    cpg.method(methodRegex).l.map { method =>
      ...
      (Some(methodFile), cedgeEntries, cvertexEntries, pedgeEntries, pvertexEntries)
    }
  }
}
...
@main def main()= {
  ...
  val dirPath = "raw_result//good_cfg_pdg"
