...
type r = (Option[String], List[cEdgeEntry], List[cVertexEntry])
...
def result(methodRegex: String = ""): List[r] = {
  if (methodRegex.isEmpty) {
    ...
    List((None, cedgeEntries, cvertexEntries))
  } else {
    cpg.method(methodRegex).l.map { method =>
      ...
      (Some(methodFile), cedgeEntries, cvertexEntries)
    }
  }
}
