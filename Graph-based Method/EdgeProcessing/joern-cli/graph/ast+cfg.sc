...
type r = (Option[String], List[aEdgeEntry], List[aVertexEntry], List[cEdgeEntry], List[cVertexEntry])
...
def result(methodRegex: String = ""): List[r] = {
  if (methodRegex.isEmpty) {
    ...
    List((None, aedgeEntries, avertexEntries, cedgeEntries, cvertexEntries))
  } else {
    cpg.method(methodRegex).l.map { method =>
      ...
      (Some(methodFile), aedgeEntries, avertexEntries, cedgeEntries, cvertexEntries)
    }
  }
}
...
@main def main()= {
  ...
  val dirPath = "raw_nvd//bad_ast_cfg"
