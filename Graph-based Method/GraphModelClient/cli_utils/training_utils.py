...
import GraphModelClient.data.data.data_preprocess as DataSplit
...
def run_train_from_args(args, hyperdrive_hyperparameter_overrides: Dict[str, str] = {}) -> None:
    ...
    if args.run_test:
        ...
        # second path
        data_path_2 = RichPath.create(
            os.path.split(args.data_path)[0] + '/tem_' + os.path.split(args.data_path)[1] + '/new/cdfg',
            args.azure_info)
        ...
        if args.run_test:
            ...
            test_data_1 = dataset.get_tensorflow_dataset(DataFold.TEST)
            test_data_2 = dataset2.get_tensorflow_dataset(DataFold.TEST)
            # log(f"Test dataset: {len(test_data_1)}")
            _, _, test_results = model.run_one_epoch_new(test_data_1, test_data_2, training=False, quiet=args.quiet)
    ...
    log(f"Deal data time {time2 - time1} s.")
    log(f"Train time {time3 - time2} s.")
    log(f"Test time {time4 - time3} s.")
    log(f"Total time {time4 - time1} s.")
