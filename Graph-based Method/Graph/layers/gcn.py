...
class GCNInput(NamedTuple):
    """Input named tuple for the GCN."""
...
class GCN(tf.keras.layers.Layer):
    ...
    def build(self, tensor_shapes: GCNInput):
        ...
        with tf.name_scope(f"{self._message_passing_class.__name__}_GCN"):
            # Then we construct the layers themselves:
            with tf.name_scope("gcn_initial_node_projection"):
    ...
    def call(self, inputs: GCNInput, training: bool = False, return_all_representations: bool = False):
    ...
    def _internal_call(self, inputs: GCNInput, training: bool = False):
